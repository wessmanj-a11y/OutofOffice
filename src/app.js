...
{name:'Director of Chaos',icon:'🔥',power:'Combos hit harder.',need:7},
{name:'Out Of Office Ascended',icon:'🌌',power:'Win by escaping corporate gravity.',need:10}
];
const cards=[
...
function start(){game={rank:0,burnout:0,sanity:5,xp:0,persona:0,rival:{rank:1,burnout:0,sanity:6},hand:[draw(),draw(),draw(),draw(),draw()],played:[],log:['Choose cards to build combos, evolve your Persona, then End Turn.'],over:false};safeHtml('result','');safeHtml('pack','');safeText('combo','None');render();}
function combo(){const names=game.played.slice(-2).map(c=>c.name);const tags=game.played.flatMap(c=>c.tags||[]);let bonus='None';if(names.includes('Work From Home')&&names.includes('Quiet Quit Ritual')){game.sanity+=4;game.burnout=Math.max(0,game.burnout-2);bonus='Ghost Employee';}else if(tags.filter(t=>t==='career').length>=2){game.rank=Math.min(positions.length-1,game.rank+1);game.xp+=2;bonus='Career Ascension';}else if(tags.filter(t=>t==='pto').length>=2){game.sanity+=2;game.xp+=1;bonus='PTO Chain';}else if(tags.filter(t=>t==='chaos').length>=2){game.rival.burnout+=3;bonus='Corporate Death Spiral';}safeText('combo',bonus);if(bonus!=='None')game.log.unshift('COMBO: '+bonus+' triggered.');}
...
function playCard(i){if(game.over)return;const c=game.hand[i];if(!c)return;let burn=c.burnout;if(c.type==='memo'&&game.persona>=2)burn=Math.max(0,burn-1);game.rank=Math.min(positions.length-1,game.rank+c.rank);game.burnout=Math.max(0,game.burnout+burn);game.sanity=Math.max(0,game.sanity+c.sanity+(c.type==='position'&&game.persona>=1?1:0));game.xp+=c.rarity==='Epic'?2:c.rarity==='Rare'?1:0;game.played.push(c);game.log.unshift(c.name+': '+c.flavor);game.hand.splice(i,1);combo();evolve();check();render();}
function endTurn(){if(game.over)return;game.hand.push(draw());const move=Math.random();if(move>.62){game.rival.rank=Math.min(positions.length-1,game.rival.rank+1);game.log.unshift('Rival played Promotion Ladder Scam.');}else if(move>.32){game.burnout+=2;game.log.unshift('Rival played Emergency Alignment Meeting.');}else{game.rival.sanity+=2;game.log.unshift('Rival played Delegation Shield.');}game.played=[];safeText('combo','None');check();render();}
function openPack(){const pulls=[draw(),draw(),draw()];game.hand.push(...pulls);safeHtml('pack',pulls.map(c=>cardHtml(c,-1)).join(''));game.log.unshift('Opened a booster pack: '+pulls.map(c=>c.name).join(', '));render();}
...
function render(){const p=personas[game.persona];safeText('burnout',game.burnout);safeText('sanity',game.sanity);safeText('rivalPosition',positions[game.rival.rank]);safeText('rivalStats','Burnout '+game.rival.burnout+' • Sanity '+game.rival.sanity);safeHtml('persona','<div class="personaCard"><div class="personaIcon">'+p.icon+'</div><div><h2>'+p.name+' Lv.'+(game.persona+1)+'</h2><p>'+p.power+'</p><b>XP '+game.xp+'</b></div></div>');safeHtml('ladder',positions.map((x,i)=>'<div class="'+(i===game.rank?'active':i===game.rival.rank?'rival':'rung')+'">'+x+'</div>').join(''));safeHtml('hand',game.hand.map((c,i)=>cardHtml(c,i)).join(''));safeHtml('log',game.log.slice(0,10).map(x=>'<p>'+x+'</p>').join(''));}
